...
export default function PracticeWorkspace({ id }: { id: string }) {
  ...
  return <div className="practice-page"><Link href={`/tutorial/${id}/${tutorial.slug}`} className="player-breadcrumb"><ArrowLeft size={15} />Back to the 3D tutorial</Link><header className="practice-heading"><div><div className="eyebrow"><span className="live-dot" />CAMERA PRACTICE</div><h1>Follow along in your space</h1><p>{tutorial.title} · Step {index + 1} of {steps.length}</p></div><button className="button button-secondary" disabled={busy || tutorial.isExample || !session} title={!session && !tutorial.isExample ? "Open your camera to start a practice session first" : undefined} onClick={() => void (voice.status === "connected" || voice.status === "connecting" ? voice.stop() : voice.start())}><Mic size={16} />{voice.status === "connected" ? "End conversation" : voice.status === "connecting" ? "Cancel connection" : "Ask by voice"}</button></header>
    {session?.status === "completed" ? <section className="practice-complete"><CircleCheck size={46} /><h2>You did that.</h2><p>You’ve worked through every step. Your progress is saved, and your tutor is here whenever you need a refresher.</p><Link className="button button-primary" href="/library">Back to my library<ArrowRight size={16} /></Link><Link className="button button-secondary" href={`/tutorial/${id}/${tutorial.slug}`}>Watch again</Link></section> : <div className="practice-layout"><section><div className="practice-camera" ref={cameraBox}><video ref={video} muted playsInline />
      ...
      {tutorial.isExample ? <><div className="eyebrow">A PREVIEW OF PRACTICE MODE</div><h2>Try the camera preview</h2><p>This example shows how the player works. To guide your hands accurately, we need a scan of your own room and equipment.</p><p>Start with a short walkthrough. Your tutor will ask for close-ups and measurements where needed.</p><Link className="button button-primary" href="/create">Get help in my space<ArrowRight size={16} /></Link></> : !calibration ? <><div className="eyebrow">ALIGN YOUR WORKSPACE</div><h2>Match the landmarks.</h2><p>Find the highlighted point in your 3D scene, then tap the same point in the camera view. Match all eight points to place the guide accurately. If your phone moves, you’ll need to align it again.</p><div className="alignment-progress">{Array.from({length:8}, (_, i) => <span className={i < points.length ? "done" : ""} key={i} />)}</div><div className="alignment-preview"><SceneViewer tutorial={tutorial} time={0} mode="free" landmarkIndex={points.length} /></div><p><strong>{Math.min(points.length + 1, 8)} of 8:</strong> {landmarks[points.length]?.label || "Waiting for scene landmarks"}</p><button className="button button-secondary" disabled={!points.length || busy} onClick={() => setPoints([])}><RotateCcw size={15} />Start alignment again</button>{landmarks.length < 8 && <p className="player-inline-error">This scene needs eight supported landmarks before spatial guidance can begin.</p>}</> : <><div className="eyebrow">STEP {String(index + 1).padStart(2,"0")} / {String(steps.length).padStart(2,"0")}</div><h2>{step?.title}</h2><p>{step?.instruction}</p>{!anchorsConfirmed ? <><p>Confirm each object’s base before this gesture. Keep objects on the same surface and facing the same direction as in your scan.</p><button className="button button-primary" disabled={busy} onClick={() => setAnchorIndex(0)}><Crosshair size={16} />Locate {movable.length} object{movable.length === 1 ? "" : "s"}</button></> : <><button className="button button-primary" disabled={busy || !active || !cameraOn} onClick={() => void checkStep()}><Camera size={16} />{busy ? "Checking…" : "Check this step"}</button><button className="button button-secondary" disabled={busy || !active || !cameraOn} onClick={() => void runAction("confirm")}><Check size={16} />I’ve done this</button><button className="button button-secondary" disabled={busy || !cameraOn} onClick={() => void runAction("repeat")}><RotateCcw size={15} />Show this step again</button><button className="button button-quiet" disabled={busy || !cameraOn} onClick={() => void runAction(active ? "pause" : "resume")}>{active ? <Pause size={15} /> : <Play size={15} />}{active ? "Pause guidance" : "Resume guidance"}</button></>}{feedback?.stepId === step?.id && <div className="practice-feedback"><strong>{feedback.status === "complete" ? "That looks right" : feedback.status === "uncertain" ? "Let’s take another look" : "A little more to do"}</strong><p>{feedback.guidance}</p></div>}<details className="practice-settings"><summary>Guidance settings</summary><div className="practice-options"><label><input type="checkbox" checked={autoCheck} onChange={event => setAutoCheck(event.target.checked)} />Check progress automatically</label><label>Overlay visibility<input type="range" min={.2} max={.9} step={.05} value={opacity} onChange={event => setOpacity(Number(event.target.value))} /></label><button className="button button-quiet" onClick={() => invalidate("Ready to realign your workspace.")}><Crosshair size={15} />Realign workspace</button></div></details></>}
    </aside></div>}
  </div>;
}
