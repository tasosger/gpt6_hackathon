  ...
  return <div className="player-page">
    ...
    <div className="tutorial-grid"><section className="tutorial-stage">
      <div className="scene-viewport" ref={viewport}>
        <div className="scene-toolbar"><span className="scene-live-label"><span />{tutorial.isExample ? "EXAMPLE SCENE" : "YOUR 3D WORKSPACE"}</span><details className="scene-view-options"><summary>View options</summary><div className="scene-view-options-content"><div className="camera-switch">{(["third", "first", "free"] as const).map(view => <button key={view} className={mode === view ? "active" : ""} aria-pressed={mode === view} onClick={() => setMode(view)}>{view === "third" ? "Overview" : view === "first" ? "Close-up" : "Explore"}</button>)}</div>{hasDetail && <button className="scene-detail-button" disabled={!sceneReady || busy} aria-pressed={detailed} aria-label={detailed ? "Use standard detail" : "More detail"} title={detailed ? "Use standard detail" : "Load the more detailed scene"} onClick={() => void toggleDetail()}><Eye size={15} /><span className="scene-detail-label">{detailed ? "Standard detail" : "More detail"}</span></button>}</div></details><button className="scene-expand" aria-label="Fullscreen tutorial" onClick={() => void fullscreen()}><Expand size={17} /></button></div>
      ...
      <div className="tutor-conversation"><div className="tutor-avatar"><Sparkles size={19} /></div><div><strong>Need help with this step?</strong><p>{voice.transcript || "Ask your tutor by voice, or use replay to see this step again."}</p>{(voice.error || notice) && <p className="player-inline-error" role="status">{voice.error || notice}</p>}</div><button className={`button ${voice.status === "connected" ? "voice-active" : "button-secondary"}`} onClick={() => { setPlaying(false); if (tutorial.isExample) { setNotice("Live voice connects to your own saved tutorial. Create one to talk through your workspace with Astra."); return; } if (voice.status === "connected" || voice.status === "connecting") voice.stop(); else void voice.start(); }}><Mic size={16} />{voice.status === "connecting" ? "Cancel connection" : voice.status === "connected" ? "End conversation" : "Ask by voice"}</button></div>
    ...
    </section><aside className="steps-panel"><div className="steps-panel-header"><span>STEP-BY-STEP GUIDE</span><span>{String(stepIndex + 1).padStart(2, "0")} / {String(steps.length).padStart(2, "0")}</span></div><div className="steps-list">{steps.map((item, index) => <button key={item.id} className={`tutorial-step ${index === stepIndex ? "current" : ""}`} aria-current={index === stepIndex ? "step" : undefined} onClick={() => seek(timing[index]?.startTime ?? 0)}><span className="step-number">{String(index + 1).padStart(2, "0")}</span><span><strong>{item.title}</strong>{index === stepIndex && <small>{item.instruction}</small>}</span>{index === stepIndex && <span className="step-playing-dot" />}</button>)}</div><div className="step-navigation"><button onClick={() => seek(timing[Math.max(0, stepIndex - 1)]?.startTime ?? 0)} disabled={stepIndex === 0}><ChevronLeft size={16} />Previous</button><button onClick={() => seek(timing[Math.min(steps.length - 1, stepIndex + 1)]?.startTime ?? 0)} disabled={stepIndex === steps.length - 1}>Next step<ChevronRight size={16} /></button></div><div className="ready-card"><div className="ready-icon"><ScanLine size={24} /></div><h2>Try it in your space</h2><p>Set up your camera and align the guide with your workspace.</p><>{tutorial.visibility === "public" && !tutorial.isExample && !isOwner ? <button className="button button-primary" disabled={busy} onClick={() => void adapt()}>Adapt to my space<ArrowRight size={17} /></button> : <Link className="button button-primary" href={`/tutorial/${tutorial.id}/${tutorial.slug}/practice`}>Start camera practice<ArrowRight size={17} /></Link>}</><small>Requires camera setup and alignment</small></div></aside></div>
