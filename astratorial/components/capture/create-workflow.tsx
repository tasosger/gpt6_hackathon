...
const steps = ["Your goal", "Show your space", "Review the plan", "Create your guide"];
...
export function CreateWorkflow({ initialTutorialId }: { initialTutorialId?: string }) {
  ...
  const entryGoal = useRef<string | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    async function restore() {
      if (initialTutorialId) {
        ...
        try {
          if (entryGoal.current === undefined) {
            const entry = JSON.parse(sessionStorage.getItem("astratorial-help-entry-v1") ?? "null");
            entryGoal.current = typeof entry?.goal === "string" && Date.now() - entry.createdAt < 10 * 60_000 ? entry.goal.slice(0, 3000) : "";
          }
          if (entryGoal.current && active) { setGoal(entryGoal.current); setStep(1); return; }
          const draft = JSON.parse(localStorage.getItem("astratorial-draft-v1") ?? "null"); if (draft?.goal && active) { setGoal(draft.goal); setTitle(draft.title ?? ""); setConstraints(draft.constraints ?? ""); setReferenceUrls(draft.referenceUrls ?? ""); } } catch { /* Ignore an obsolete draft. */ }
      }
    }
    void restore();
  ...
  function clearEntry() { sessionStorage.removeItem("astratorial-help-entry-v1"); entryGoal.current = ""; }
  function saveLocal() { localStorage.setItem("astratorial-draft-v1", JSON.stringify({ version: 1, goal, title, constraints, referenceUrls })); clearEntry(); setSaved(true); }
  async function saveDetails(current = tutorial, includeAnswers = false): Promise<Tutorial | null> {
    ...
    if (!current) {
      ...
      acceptTutorial(result.tutorial); const draftUrl = new URL(window.location.href); draftUrl.searchParams.delete("tutorial"); draftUrl.searchParams.set("id", result.tutorial.id); window.history.replaceState(window.history.state, "", draftUrl); clearEntry(); setSaved(true); return result.tutorial;
    }
    ...
    if (Object.keys(changes).length === 0) { clearEntry(); setSaved(true); return current; }
    ...
    acceptTutorial(result.tutorial); if (includeAnswers) setAnswers({}); clearEntry(); setSaved(true); return result.tutorial;
  }
  ...
  return <div className="create-page"><div className="page-heading create-heading"><div><span className="eyebrow">LET’S MAKE SOMETHING POSSIBLE</span><h1>Let’s make your next step clear.</h1><p>Start with your goal, add your captures, and build a guide for your space.</p></div><button className="button button-secondary" disabled={!!busy || running || !goal.trim()} onClick={() => void run("save", async () => { await saveDetails(); })}>{busy === "save" ? <span className="spinner" /> : <Icon name="file" size={15} />}{saved ? "Draft saved" : "Save draft"}</button></div>
    ...
    <nav className="workflow-stepper" aria-label="Create tutorial progress">{steps.map((label, index) => <button key={label} className={step === index ? "active" : index < step ? "completed" : ""} onClick={() => setStep(index)} disabled={!!busy || running || (index !== step && ((index === 1 && !goal.trim()) || (index === 2 && !tutorial?.plan && job?.kind !== "analyze") || (index === 3 && !tutorial?.scene && job?.kind !== "generate" && (!tutorial?.plan || !measurementsReady || tutorial.plan.questions.some((question) => question.required)))))} aria-current={step === index ? "step" : undefined}><span className="step-number">{index < step ? <Icon name="check" size={12} /> : index + 1}</span>{label}</button>)}</nav>
