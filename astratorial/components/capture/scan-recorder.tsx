...
export function ScanRecorder({ mode, onCapture, onClose }: { mode: "video" | "audio"; onCapture: (file: File) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    async function open() {
      try {
        ...
        const media = await navigator.mediaDevices.getUserMedia(mode === "video" ? { video: { facingMode: { ideal: "environment" }, width: { ideal: 1920 }, height: { ideal: 1080 } }, audio: true } : { audio: true });
  ...
  function start() {
    ...
    try {
      ...
      const current = new MediaRecorder(stream.current, mime ? { mimeType: mime } : undefined);
      const chunks: Blob[] = [];
