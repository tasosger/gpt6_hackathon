...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <ConfigContext value={{ config, loading, error, refresh }}>
    ...
    <aside id="app-navigation" className={`sidebar ${menuOpen ? "is-open" : ""}`}>
      ...
      <Link href="/create" className="button button-primary sidebar-create" onClick={() => setMenuOpen(false)}><Icon name="plus" size={18} /> Create a tutorial</Link>
      ...
      <div className="sidebar-bottom"><Link className={`nav-link ${pathname === "/settings" ? "active" : ""}`} href="/settings" onClick={() => setMenuOpen(false)}><Icon name="settings" /><span>Settings</span>{!loading && !config?.configured && <span className="service-dot" />}</Link><Link className="profile-link" href={config?.user ? "/settings" : "/login"} onClick={() => setMenuOpen(false)}><span className="avatar">{config?.user?.email?.slice(0, 1).toUpperCase() ?? <Icon name="user" size={18} />}</span><span><strong>{config?.user ? config.user.email.split("@")[0] : "Your personal space"}</strong><small>{config?.user ? "Account settings" : "Sign in to save your tutorials"}</small></span><Icon name="chevron" size={15} /></Link></div>
    </aside>
