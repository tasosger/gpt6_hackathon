...
Enable **Authentication → Sign In / Providers → Allow anonymous sign-ins**, then **Save changes**. Everyone is an anonymous guest; the app creates a session automatically when an upload or shared-tutorial adaptation needs private storage. Each guest receives a distinct owner ID with row-level isolation. Set the Auth Site URL to `http://localhost:4173` for the local demo. The app has no email sign-in or settings flow. Guest access persists in that browser's cookies; clearing them loses access to its library.
...
Source uploads reserve at most **50 MB/file, 30 files and 200 MB/tutorial, 500 MB/guest and 500 MB/project**. PDF manuals together must fit within **10 MB/tutorial**. These use decimal bytes. SQL serializes reservations across the project, including parallel uploads by different guests. Each pending upload reserves the full 50 MB bucket ceiling because signed tokens cannot constrain declared content length. On completion the server verifies actual size and settles the reservation down. Abandoned reservations remain counted until tutorial deletion. Generated assets and bandwidth use additional quota; source limits are not a total storage invoice cap.
...
- `GET /api/config` exposes service readiness and the current guest's ID, never credentials or email. Missing service configuration returns actionable `503` responses for dependent actions.
- `POST /api/auth/guest` starts a guest session when needed and reuses an existing session. Email OTP, verification and logout endpoints are unavailable.
...
Run `npm run test:database` for disposable local PostgreSQL checks; it never targets the configured project. Then test the actual browser → Storage → queue → worker flow and isolation between two separate guest browser sessions against your Supabase project. See [the verification record](verification.md) for the latest connected-test status. Unit tests do not establish physical-phone performance, illustration usefulness or actual provider invoices.
