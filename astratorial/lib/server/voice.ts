...
export function requireWorkerToken(request: Request) {
  ...
  const expected = `Bearer ${process.env.LOCAL_WORKER_TOKEN || ""}`;
  if (!process.env.LOCAL_WORKER_TOKEN || provided.length !== expected.length || !timingSafeEqual(Buffer.from(provided), Buffer.from(expected))) fail(401, "Worker authorization required.");
}
export async function startVoice(ownerId: string, tutorial: Tutorial, input: z.infer<typeof voiceInput>) {
  ...
  try {
    ...
    const guarded = await fetch(process.env.LOCAL_VOICE_URL!, { method:"POST",headers:{Authorization:`Bearer ${process.env.LOCAL_WORKER_TOKEN}`,"Content-Type":"application/json"},body:JSON.stringify({voiceSessionId:id,callId,maxSeconds:VOICE_MAX_SECONDS,budgetUsd:VOICE_BUDGET_USD}),signal:AbortSignal.timeout(20_000) });
