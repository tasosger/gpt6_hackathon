...
Capture reservations allow 200 MB per tutorial, 500 MB per account, and 500 MB across the project. An unfinished signed ticket reserves the full 50 MB until its actual size is verified; delete unused tutorials to reclaim their files and reservations. This leaves part of the Free plan's included 1 GB storage for generated scenes, but derived assets and bandwidth still need monitoring in the dashboard. The local illustrated worker also rejects individual derived assets above the 50 MB bucket limit.
