...
import { PLAN_INSTRUCTIONS,SCENE_INSTRUCTIONS,GENERATION_REASONING,ANALYSIS_REASONING,PLAN_OUTPUT_TOKENS,SCENE_OUTPUT_TOKENS,validatePlanIds } from '../lib/local/pipeline';
...
async function main(){
  ...
  const result=await ai.responses.stream({model:process.env.OPENAI_MODEL||'gpt-6-astra',store:false,max_output_tokens:PLAN_OUTPUT_TOKENS,reasoning:{effort:ANALYSIS_REASONING},instructions:PLAN_INSTRUCTIONS,input:[{role:'user',content:[{type:'input_text',text:'No goal has been entered. Infer the most likely task from this synthetic test image. This is a fixture, not real user footage. Do not invent external sources; there is no web search in this isolated smoke test.'},content]}],text:{format:zodTextFormat(TutorialPlanSchema,'tutorial_plan')}}).finalResponse();
  ...
  const scene=await ai.responses.stream({model:process.env.OPENAI_MODEL||'gpt-6-astra',store:false,max_output_tokens:SCENE_OUTPUT_TOKENS,reasoning:{effort:GENERATION_REASONING},instructions:SCENE_INSTRUCTIONS,input:[{role:'user',content:[{type:'input_text',text:JSON.stringify({plan})},content]}],text:{format:zodTextFormat(IllustrationSchema,'tutorial_illustration')}}).finalResponse();
