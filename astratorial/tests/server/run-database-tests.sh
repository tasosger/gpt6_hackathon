...
for TASK_MIGRATION_PATH in "$TASK_TEST_ROOT"/supabase/migrations/*.sql; do
  sed '/create extension if not exists pgmq;/d' "$TASK_MIGRATION_PATH" > "$TASK_PG_DIR/migration.sql"
