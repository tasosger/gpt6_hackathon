> **Preserved experimental prototype** — This standalone photo-based app comes from
> `feat/scene-generator` at commit `9bbf959` (including its earlier work). The main
> Astratorial application remains in `../../astratorial` and keeps the video-first
> flow. This prototype does not include that application's guest access, persistence,
> generation budgets, or local worker. Its paid generation endpoint has no guest or
> budget protections; run it locally rather than exposing it publicly.

Run this prototype separately, using Node 24+ and its own server-side configuration:

```bash
cd prototypes/scene-generator
npm ci
cp .env.example .env.local
# Set your own OPENAI_API_KEY in this prototype's .env.local.
npm test
npm run dev -- --port 4180
```

Open http://localhost:4180. The local example works without a key; generating a new
photo walkthrough requires access to the configured OpenAI model and incurs API
usage. The primary app's `.env.local` is not loaded here. Do not commit credentials.
